...
type Media struct {
    ...
    PresignedUrl480		string `json:"presigned_url_480"`
    PresignedUrlThumb480	string `json:"presigned_url_thumb_480"`
...
func Main(w http.ResponseWriter, r *http.Request) {
    ...
    mediaFile.PresignedUrl480 = r.FormValue("presigned_url_480")
    mediaFile.PresignedUrlThumb480 = r.FormValue("presigned_url_thumb_480")
    ...
    ffmpegFileLink480 := ffmpeg.VideoPreview480(tempFile.Name(), ffmpegFileNameWithoutExtension, w)
    uploadErrorVideo480, uploadErrorVideo480msg := amazon.UploadByPresignedUrl(mediaFile.PresignedUrl480, ffmpegFileLink480, "video/mp4")
    if uploadErrorVideo480 {
		helpers.GetError(w, uploadErrorVideo480msg)
		return
    }
    // END 480

    // BEGIN THUMB 480: create 480 thumbnail, upload to aws s3
    ffmpegFileLinkThumb480 := ffmpeg.VideoPreviewThumbnail480(ffmpegFileLink480, ffmpegFileNameWithoutExtension, w)
    uploadErrorThumb480, uploadErrorThumb480msg := amazon.UploadByPresignedUrl(mediaFile.PresignedUrlThumb480, ffmpegFileLinkThumb480, "image/jpeg")
    if uploadErrorThumb480 {
		helpers.GetError(w, uploadErrorThumb480msg)
		return
    }
    // END THUMB 480
    ...
    helpers.RemoveFile(ffmpegFileLink480, w)
    helpers.RemoveFile(ffmpegFileLinkThumb480, w)
